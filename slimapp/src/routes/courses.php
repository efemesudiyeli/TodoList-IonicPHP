...
$app->post('/kayit', function (Request $request, Response $response) {
    ...
    try{        
        ...
        $password_check = preg_match('~^[A-Za-z0-9!@#$%^&*()_]{6,20}$~i', $passwordr);
         ...
         if (true /* strlen(trim($username))>0 && strlen(trim($passwordr))>0 && strlen(trim($email))>0 && $email_check>0 && $username_check>0 && $password_check>0 */ )
        {
        ...
        try{
            ...
            if($mesaj){
                return $response
                    ...
                    ->withHeader("Content-Type", 'application/json')
                    ->withJson(array(
                        "text"  => "Görev başarılı bir şekilde güncellenmiştir.."
                    ), null, JSON_UNESCAPED_UNICODE | JSON_NUMERIC_CHECK);
    ...
    try{
        ...
        if($mesaj){
            return $response
                ...
                ->withHeader("Content-Type", 'application/json')
                ->withJson(array(
                    "text"  => "Görev başarılı bir şekilde silinmiştir.."
                ));
